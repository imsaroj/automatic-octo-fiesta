...
describe("mergeServerFilters", () => {
  const base: ServerFilter[] = [{ field: "a", filterType: "text", type: "contains", value: "x" }];
  const external: ServerFilter[] = [{ field: "b", filterType: "text", type: "equals", value: "y" }];
...
describe("persisted grid state", () => {
  it("round-trips column + filter state through localStorage", () => {
    ...
    expect(parsed?.filterModel).toEqual({ name: { type: "contains", filter: "ada" } });
  });
  ...
  it("never throws when storage is unavailable", () => {
    ...
    expect(() => writePersistedGridState("k", { columnState: [] })).not.toThrow();
  });
});
...
function fakeExportApi(rows: Row[]): GridApi<Row> {
  ...
  return {
    ...
    forEachNode: (cb: (node: { data: Row | undefined; id: string }) => void) => {
...
describe("collectGridExport", () => {
  it("builds headers from displayed columns (headerName, else colId)", () => {
    const table = collectGridExport(fakeExportApi([{ id: 1, name: "Ada", mrr: 2500 }]));
  ...
  it("coerces nullish cell values to an empty string", () => {
    const table = collectGridExport(fakeExportApi([{ id: 1, name: "", mrr: 0 }]));
