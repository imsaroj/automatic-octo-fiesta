...
export interface SmartServerGridProps<TRow> {
   ...
   */
  fetchRows: (params: ServerFetchParams, signal: AbortSignal) => Promise<ServerFetchResult<TRow>>;
...
function ErrorPanel({ message, onRetry }: { message: string; onRetry: () => void }) {
  return (
    <div className="bg-background/80 absolute inset-0 z-10 flex items-center justify-center backdrop-blur-sm">
      <div className="border-border bg-card flex max-w-sm flex-col items-center gap-3 rounded-md border p-6 text-center shadow-sm">
...
function SmartServerGridInner<TRow>(
  ...
  const restoreState = useCallback((api: GridApi<TRow>) => {
    ...
    if (!parsed) return;
    if (parsed.columnState) api.applyColumnState({ state: parsed.columnState, applyOrder: true });
    ...
    () => ({
      getRows: (params: IGetRowsParams) => {
        const serverParams = buildServerFetchParams({
          ...
          filterModel: (params.filterModel ?? null) as Record<string, unknown> | null,
        });
        // Merge any external (search-form) filters on top of column filters.
        serverParams.filters = mergeServerFilters(serverParams.filters, filtersRef.current);
  ...
  const handleRowDoubleClicked = useCallback((event: RowDoubleClickedEvent<TRow>) => {
  ...
  const handleExport = useCallback(() => {
    ...
    const sheetName = typeof title === "string" && title.length > 0 ? title : "Export";
    downloadXlsx(`${exportFileName}-${timestampForFilename()}`, { name: sheetName, headers, rows });
  ...
  const showToolbar =
    title || refreshable || columnSelector || exportExcel || toolbarActions || selection !== "none";
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        {initialLoading && !error ? <SmartLoadingOverlay loading label="Loading data…" /> : null}
...
export const SmartServerGrid = SmartServerGridForwarded as <TRow>(
  props: SmartServerGridProps<TRow> & { ref?: ForwardedRef<SmartServerGridHandle<TRow>> },
) => ReactElement;
