...
describe("normalizeFilterModel", () => {
  ...
  it("normalizes a text filter", () => {
    ...
    expect(filters).toEqual([
      { field: "name", filterType: "text", type: "contains", value: "John", valueTo: undefined },
    ]);
  });

  it("normalizes a number range filter", () => {
    const filters = normalizeFilterModel({
      mrr: { filterType: "number", type: "inRange", filter: 1000, filterTo: 2000 },
    });
  ...
  it("collapses a combined (AND/OR) condition to its first predicate", () => {
    ...
    expect(filters[0]).toMatchObject({ field: "mrr", type: "greaterThan", value: 500 });
  });
});
