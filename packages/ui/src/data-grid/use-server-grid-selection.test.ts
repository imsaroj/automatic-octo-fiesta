...
function fakeApi(nodesRef: { current: FakeNode[] }): GridApi<Row> {
  return {
    forEachNode: (cb: (node: FakeNode) => void) => nodesRef.current.forEach((n) => cb(n)),
...
describe("useServerGridSelection", () => {
  it("tracks user selections and emits loaded rows + all ids", () => {
    ...
    const apiRef: RefObject<GridApi<Row> | null> = { current: fakeApi(nodesRef) };
    const onSelectionChange = vi.fn();
    const { result } = renderHook(() => useServerGridSelection(apiRef, onSelectionChange));
    ...
    expect(onSelectionChange).toHaveBeenLastCalledWith({ ids: ["1"], rows: [node1.data] });
  });

  it("keeps the id set as the source of truth across block reloads", () => {
    const r1 = { id: 1, name: "Ada" };
    let nodesRef = { current: [makeNode(r1), makeNode({ id: 2, name: "Grace" })] };
    // Mutable on purpose — we swap the api as "blocks" reload below.
    const apiRef: { current: GridApi<Row> | null } = { current: fakeApi(nodesRef) };
    const onSelectionChange = vi.fn();
    const { result } = renderHook(() => useServerGridSelection(apiRef, onSelectionChange));
    ...
    act(() => {
      nodesRef.current = [makeNode({ id: 3, name: "Linus" }), makeNode({ id: 4, name: "Bjarne" })];
