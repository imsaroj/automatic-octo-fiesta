...
import type { ServerFetchResult } from "@/api/pagination";
...
describe("SmartServerGrid", () => {
  it("renders the title and default toolbar controls", () => {
    render(
      <SmartServerGrid title="Users" columns={columns} fetchRows={fetchRows} getRowId={getRowId} />,
    );
  ...
  it("mounts with external filters, fill layout and a double-click handler", () => {
    render(
      <SmartServerGrid
        ...
        fetchRows={fetchRows}
        getRowId={getRowId}
        filters={[{ field: "name", filterType: "text", type: "contains", value: "ada" }]}
        columnFilters={false}
        fill
        onRowDoubleClick={() => {}}
      />,
